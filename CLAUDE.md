...
- **Adjust `===`/`---`/`^^^` underlines to match translated title length** — this is the #1 cause of post-sync build warnings. German/French titles are typically 30-50% longer than English. CJK chars ≈ 2 ASCII for underline width. Always add +2 margin. **Count characters, not bytes.**
...
**⚠️ CRITICAL: Run this step AFTER every round of edits that modify or add section titles** — not just once at the end. Adding anchor numbering (e.g. `Configuring` → `1. Configuring`), inserting anchors before titles, or any structural edits can introduce new underline mismatches.

**Why underlines break repeatedly:**
1. Translation agents copy English underline lengths but don't adjust for longer translated titles (German/French are 30-50% longer than English)
2. Post-translation edits (adding `1.` numbering, anchors, fixing refs) modify titles without updating underlines
3. The auto-fix runs once, then subsequent edits reintroduce short underlines
