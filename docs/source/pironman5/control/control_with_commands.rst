...
.. note::
   ...
   Chaque fois que vous modifiez l'état de ``pironman5.service``, vous devez redémarrer le service pour que les modifications de configuration prennent effet.
...
* Pour changer la couleur des LED RGB, saisissez la valeur hexadécimale souhaitée, telle que ``fe1a1a``.
...
* Pour changer les modes d'affichage des LED RGB, choisissez parmi :

  ``solid`` / ``breathing`` / ``flow`` / ``flow_reverse`` / ``rainbow`` / ``rainbow_reverse`` / ``hue_cycle``

  .. note::

     Si le mode d'affichage des LED RGB est réglé sur ``rainbow``, ``rainbow_reverse`` ou ``hue_cycle``, le réglage de couleur via ``pironman5 -rc`` ne sera pas pris en compte.

  .. code-block:: shell

     sudo pironman5 -rs breathing

* Pour modifier la vitesse d'animation des LED RGB (plage : ``0 ~ 100``) :

  .. code-block:: shell

     sudo pironman5 -rp 80

* La configuration par défaut comprend 4 LED RGB. Si vous connectez des LED supplémentaires, mettez à jour le nombre de LED avec :
...
* Vous pouvez configurer le mode de fonctionnement des deux ventilateurs GPIO à l'aide de commandes. Ces modes déterminent le seuil de température auquel les ventilateurs GPIO s'activeront.
