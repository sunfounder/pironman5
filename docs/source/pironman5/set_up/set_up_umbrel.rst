...
如果你在 Raspberry Pi 5 上安装了 Umbrel OS，则需要使用命令行来配置 Pironman 5。以下是详细步骤说明：
...
#. 你可以选择在 Umbrel OS 内或在特定应用内打开终端。无论哪种方式，都会进入命令行界面。
...
#. 重启后，``pironman5.service`` 服务会自动启动。  
   以下是 Pironman 5 的主要配置：

   * OLED 屏幕显示 CPU、RAM、磁盘使用率、CPU 温度和 Raspberry Pi 的 IP 地址。  
   * 四个 WS2812 RGB 灯将以蓝色呼吸效果亮起。  
   * RGB 风扇默认设置为 **Always On（始终开启）** 模式。若需调整温度触发设置，请参阅 :ref:`cc_control_fan`。
