...
如果你有一块 NVMe SSD，但没有可将 NVMe SSD 连接到电脑的转接器，可以选择第三种方案：先将系统安装到 **Micro SD 卡** 上。当 Pironman 5 MAX 成功启动后，再将系统从 Micro SD 卡复制到 NVMe SSD。

* 首先，你需要完成 :ref:`install_os_sd_5`。
* 然后，启动并登录你的 Raspberry Pi。如果你不确定如何登录，可以访问树莓派官方网站：|link_rpi_get_start|。
...
.. * 该连接通过了 Gen 2.0（5 GT/sec）速率认证，如果你在 ``/boot/firmware/config.txt`` 中添加以下内容，也可以强制启用 Gen 3.0（10 GT/sec）。
...
* 你需要禁用 PCIe 的启动延时，这样 Raspberry Pi 才能在启动时识别位于 PCIe Switch 后面的 NVMe 硬盘。请在 ``/boot/firmware/config.txt`` 中添加以下一行：
...
如果你安装了两块 NVMe 系统盘，并需要选择其中一块作为启动盘，
可以修改 ``/boot/firmware/cmdline.txt`` 文件中的 ``ROOT=PARTUUID=xxxxxxxxx``，将其替换为你希望启动的磁盘对应的 UUID。你可以使用以下命令查看磁盘的 UUID：
